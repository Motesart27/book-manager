...
const BUILD_ID='v2.1-'+Date.now().toString(36)
...
export default function App(){
  ...
  async function toggleTask(tid,st){setUpdatingTask(tid);await apiFetch('/api/book/tasks/'+tid,{method:'PATCH',body:JSON.stringify({Status:st==='Done'?'Open':'Done'})});loadDash();setUpdatingTask(null)}
  function togglePurchased(id){setPurchased(p=>({...p,[id]:!p[id]}))}
  async function createTask(name){if(!name.trim())return;await apiFetch('/api/book/tasks',{method:'POST',body:JSON.stringify({'Task Name':name.trim(),'Status':'Open','Priority':'Medium'})});loadDash();setTaskModal(false);setTaskName('')}
  function fmt(text){return text.replace(/\*\*(.*?)\*\*/g,'<strong>$1</strong>').replace(/\n\n/g,'<br><br>').replace(/\n/g,'<br>')}
  const sortedTasks=[...(dash?.openTasks||[])].sort((a,b)=>{const p={High:0,Medium:1,Low:2};return(p[a.priority]||1)-(p[b.priority]||1)})
  ...
  const budgetTotal=BUDGET_ITEMS.reduce((s,i)=>s+i.cost,0),budgetSpent=BUDGET_ITEMS.filter(i=>purchased[i.id]).reduce((s,i)=>s+i.cost,0),budgetRemaining=budgetTotal-budgetSpent,purchasedCount=BUDGET_ITEMS.filter(i=>purchased[i.id]).length
  ...
  return(
    <div style={{background:G.bg,color:G.text,height:'100dvh',fontFamily:"'DM Sans',system-ui,sans-serif",display:'flex',flexDirection:'column',overflow:'hidden',paddingTop:'env(safe-area-inset-top)'}}>
      ...
      <div style={{flex:1,overflow:'hidden',display:'flex',flexDirection:'column'}}>
        ...
        {tab==='overview'&&<div style={{flex:1,overflowY:'auto',padding:'14px'}}>
          ...
          <div style={{padding:'14px',background:`linear-gradient(135deg,${G.gdim},${G.gglow})`,border:`1px solid rgba(201,168,76,0.3)`,borderRadius:12,marginBottom:12,display:'flex',alignItems:'center',gap:12}}>
            <span style={{fontSize:28}}>▸</span><div style={{flex:1}}><div style={{fontSize:10,color:G.gold,fontWeight:700,textTransform:'uppercase'}}>Convention Deadline</div><div style={{fontSize:16,fontWeight:800,color:G.text}}>Mid-June 2026</div></div>
        ...
        {tab==='calendar'&&<div style={{flex:1,overflowY:'auto',padding:'14px'}}>
          <div style={{background:G.s2,border:`1px solid ${G.border}`,borderRadius:10,overflow:'hidden',marginBottom:10}}>
            <div style={{padding:'10px 13px',borderBottom:`1px solid ${G.border}`,display:'flex',alignItems:'center',gap:7}}><span style={{color:G.gold}}>▦</span><span style={{fontSize:11,fontWeight:700,color:G.text,flex:1}}>Calendar</span><span style={{fontFamily:'monospace',fontSize:10,color:G.t3}}>{convDaysCalc}d to conv.</span></div>
            <div style={{padding:12}}>
              <div style={{padding:'8px 10px',background:`linear-gradient(135deg,${G.gdim},${G.gglow})`,border:`1px solid rgba(201,168,76,0.3)`,borderRadius:8,marginBottom:10,display:'flex',alignItems:'center',gap:10}}><span style={{fontSize:16}}>▸</span><div style={{flex:1}}><div style={{fontSize:9,color:G.gold,fontWeight:700,textTransform:'uppercase'}}>Convention</div><div style={{fontSize:12,fontWeight:800,color:G.text}}>Mid-June 2026</div></div><div style={{textAlign:'right'}}><div style={{fontFamily:'monospace',fontSize:18,fontWeight:700,color:G.gold2}}>{convDaysCalc}</div><div style={{fontSize:8,color:G.t3,textTransform:'uppercase'}}>days left</div></div></div>
        ...
        {tab==='budget'&&<div style={{flex:1,overflowY:'auto',padding:'14px'}}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginBottom:12}}>
            <div style={{background:G.surface,border:'1px solid '+G.border,borderRadius:10,padding:'10px'}}><div style={{fontFamily:'monospace',fontSize:16,fontWeight:700,color:G.gold}}>${budgetTotal}</div><div style={{fontSize:8,color:G.t3,textTransform:'uppercase'}}>Total</div></div>
            <div style={{background:G.surface,border:'1px solid '+G.border,borderRadius:10,padding:'10px'}}><div style={{fontFamily:'monospace',fontSize:16,fontWeight:700,color:G.green}}>${budgetSpent}</div><div style={{fontSize:8,color:G.t3,textTransform:'uppercase'}}>Spent</div></div>
            <div style={{background:G.surface,border:'1px solid '+G.border,borderRadius:10,padding:'10px'}}><div style={{fontFamily:'monospace',fontSize:16,fontWeight:700,color:G.gold}}>${budgetRemaining}</div><div style={{fontSize:8,color:G.t3,textTransform:'uppercase'}}>Left</div></div>
          </div>
          <div style={{fontSize:9,color:G.t3,letterSpacing:'.1em',textTransform:'uppercase',marginBottom:10}}>Budget {purchasedCount}/{BUDGET_ITEMS.length} purchased</div>
          {BUDGET_ITEMS.map(item=><div key={item.id} onClick={()=>togglePurchased(item.id)} style={{display:'flex',alignItems:'center',gap:10,padding:'12px 14px',background:purchased[item.id]?'rgba(77,184,122,0.06)':G.surface,border:'1px solid '+(purchased[item.id]?'rgba(77,184,122,0.3)':item.urgent?'rgba(224,85,85,0.3)':G.border),borderRadius:12,marginBottom:8,cursor:'pointer'}}><div style={{width:24,height:24,borderRadius:6,border:'2px solid '+(purchased[item.id]?G.green:G.border2),background:purchased[item.id]?G.green:'transparent',display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,color:'#000',flexShrink:0}}>{purchased[item.id]?'\u2713':''}</div><div style={{flex:1,fontSize:13,color:purchased[item.id]?G.t3:G.text,textDecoration:purchased[item.id]?'line-through':'none'}}>{item.label}{item.urgent&&!purchased[item.id]&&<span style={{marginLeft:8,fontSize:8,background:'rgba(224,85,85,0.15)',color:G.red,border:'1px solid rgba(224,85,85,0.3)',borderRadius:4,padding:'1px 5px',fontWeight:700}}>URGENT</span>}</div><div style={{fontFamily:'monospace',fontSize:13,color:purchased[item.id]?G.green:item.urgent?G.red:G.gold,fontWeight:700}}>${item.cost}</div></div>)}
        </div>}
      </div>
      {/* BOTTOM NAV */}
      ...
      {/* TASK MODAL */}
      {taskModal&&<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',display:'flex',alignItems:'flex-end',zIndex:1000}} onClick={e=>{if(e.target===e.currentTarget)setTaskModal(false)}}><div style={{background:G.s2,border:`1px solid ${G.border2}`,borderRadius:'18px 18px 0 0',padding:'20px',width:'100%',boxSizing:'border-box',paddingBottom:'calc(20px + env(safe-area-inset-bottom))'}}><div style={{width:40,height:4,borderRadius:2,background:G.border2,margin:'0 auto 18px'}}/><div style={{fontSize:17,fontWeight:800,color:G.text,marginBottom:14}}>Save as Task</div><input value={taskName} onChange={e=>setTaskName(e.target.value)} onKeyDown={e=>e.key==='Enter'&&createTask(taskName)} placeholder="What needs to be done?" autoFocus style={{width:'100%',background:G.surface,border:`1px solid ${G.border2}`,borderRadius:10,padding:'11px 14px',color:G.text,fontSize:15,outline:'none',fontFamily:'inherit',marginBottom:14,boxSizing:'border-box'}}/><div style={{display:'flex',gap:10}}><button onClick={()=>setTaskModal(false)} style={{flex:1,padding:'12px',borderRadius:10,background:G.s3,border:`1px solid ${G.border}`,color:G.t2,cursor:'pointer',fontFamily:'inherit',fontSize:14,fontWeight:700}}>Cancel</button><button onClick={()=>createTask(taskName)} style={{flex:1,padding:'12px',borderRadius:10,background:G.gold,border:'none',color:'#000',cursor:'pointer',fontFamily:'inherit',fontSize:14,fontWeight:700}}>Save Task</button></div></div></div>}
