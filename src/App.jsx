...
export default function App(){
  ...
  return(
    <div style={{background:G.bg,color:G.text,height:'100dvh',fontFamily:"'DM Sans',system-ui,sans-serif",display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      {/* BOTTOM NAV */}
      <div style={{background:G.surface,borderTop:`1px solid ${G.border}`,display:'flex',flexShrink:0,paddingBottom:'env(safe-area-inset-bottom)'}}>
        {TABS.map(t=><button key={t.id} onClick={()=>setTab(t.id)} style={{flex:1,padding:'8px 2px',background:'none',border:'none',borderTop:`2px solid ${tab===t.id?G.gold:'transparent'}`,color:tab===t.id?G.gold:G.t3,cursor:'pointer',fontFamily:'inherit',display:'flex',flexDirection:'column',alignItems:'center',gap:2}}><span style={{fontSize:18}}>{t.icon}</span><span style={{fontSize:8,fontWeight:700}}>{t.label}</span></button>)}
      </div>
      {/* TASK MODAL */}
